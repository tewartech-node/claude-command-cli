...
describe('gh command', () => {
  describe('gh open', () => {
    ...
    it('constructs correct github URL', async () => {
      ...
      expect(result.url).toBe('https://claude.ai/new?repo=https://github.com/tewartech-node/test-project');
    });
  });
  ...
  describe('gh push', () => {
    ...
    it('returns push instructions with custom message', async () => {
      ...
      expect(result.action).toBe('push_instructions');
      expect(result.instructions.some(i => i.includes('feat: new feature'))).toBe(true);
    });

    it('includes all required git commands', async () => {
      const result = await handleGh(['push', 'test commit'], {});
      expect(result.instructions).toContain('git add .');
      expect(result.instructions.some(i => i.includes('git commit'))).toBe(true);
      expect(result.instructions.some(i => i.includes('git push'))).toBe(true);
    });
  });
